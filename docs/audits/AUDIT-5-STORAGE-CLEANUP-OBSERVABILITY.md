...
- The worker emits JSON logs for completion, retry/failure, queue health, and worker-level errors.
...
| `storage_cleanup_job_completed` | The worker deleted the intended tenant-scoped object or prefix. | No action unless unexpected volume is observed. |
| `storage_cleanup_job_failed` | A deletion attempt failed. `finalAttempt: false` means BullMQ will retry; `true` means retries are exhausted. | Inspect the matching audit log and object-storage error. Requeue only after the underlying cause is corrected. |
...
- `apps/worker/test/queue-observability.test.ts` verifies health-state, threshold, and failure-event serialization behavior without requiring Redis.
...
2. Forward JSON worker logs to a persistent log system and create alerts for `storage_cleanup_queue_health.status = degraded`, `storage_cleanup_job_failed.finalAttempt = true`, and worker errors.
