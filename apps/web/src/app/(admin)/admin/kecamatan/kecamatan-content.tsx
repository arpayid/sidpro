...
import { useDistrictOverview } from '@/features/tenants/use-district-overview';
...
export function KecamatanContent() {
  const { data, isLoading, error } = useDistrictOverview();

  if (isLoading) {
    return <p className="text-sm text-slate-500">Memuat dashboard kecamatan...</p>;
  }

  if (error || !data) {
    return (
      <p className="text-sm text-red-600">
        {(error as Error)?.message ?? 'Dashboard kecamatan tidak tersedia untuk akun ini.'}
      </p>
    );
  }
  ...
  return (
    <div>
      <h1 className="page-title">Dashboard Kecamatan</h1>
      <p className="page-description">
        Ringkasan agregat desa di bawah kecamatan — {data.district.name} ({data.villageCount}{' '}
        desa).
      </p>
      ...
      <Card className="mt-6">
        <CardHeader>
          <CardTitle className="text-base">Desa di Bawah Kecamatan</CardTitle>
        </CardHeader>
        <CardContent>
